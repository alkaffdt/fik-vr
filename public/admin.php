...
<?php include("config.php"); ?>
	...
	<body>	
		<h1 align="center" >ADMIN PANEL Virtual Tour Guide FIK</h1>
						...
						<tr>
							...
							<td style="vertical-align: text-bottom;">
								<form action="/proses_edit.php">
 								<input type="text" name="nama deskripsi" value="<?php
              // Create connection
              $conn_hs_girireka2TU = new mysqli($servername, $username, $password, $dbname);
              // Check connection
              if ($conn_hs_girireka2TU->connect_error) {
                  die("Connection failed: " . $conn_hs_girireka2TU->connect_error);
              } 

              $sql_hs_girireka2TU = "SELECT * FROM `deskripsi` WHERE id_desc = 11001";
              $result_hs_girireka2TU = $conn_hs_girireka2TU->query($sql_hs_girireka2TU);

              if ($result_hs_girireka2TU->num_rows > 0) {
                  // output data of each row
                  while($row_hs_girireka2TU = $result_hs_girireka2TU->fetch_assoc()) {
                      echo $row_hs_girireka2TU["deskripsi"] ;
                  }
              } else {
                  echo "0 result_hs_girireka2TUs";
              }
              $conn_hs_girireka2TU->close();
              ?>"
 										 style="width: 313px"> 
